...
| `InMemory` | A filesystem held in the process's memory, for tests: an `InMemoryFileSystem` from the `Cap.Std.Testing` package ([Testing](testing.md)); never what `Dir.ResolutionBackend` reports | Nothing about the host. Paths beneath it are still parsed and resolved by this library, one name at a time or a whole path at once as the filesystem was told, under the path rules it was told to follow. Its handles are not kernel objects, so an operation that needs one, such as reaching a Unix-domain socket beneath the directory, fails as not supported, and `UnsafeGetHandle` on a `Dir` or `CapFile` throws `NotSupportedException`. File contents are held in memory too, and `AsStream` returns a stream of the filesystem's own rather than a `FileStream`. |
